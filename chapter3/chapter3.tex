...
\begin{table}[t]
        ...
        Sample time (realtime search) & $t_\text{s}\lesssim\qty{1}{\milli\s}$ \\
        Sample time (baseband dump) & $t_\text{s}=\qty{3.33}{\micro\s}$ \\
        Antenna beam & $\text{HPBW}_\text{a}\approx\qty{90}{\degree}$ \\
        Antenna gain & $G_a\approx\num{7}$ dBi \\
...
\begin{enumerate}
    ...
    \item \textbf{Galactic transients.} CHARTS’ daily transit over the \gls{gc} (see Figure~\ref{fig:instantaneous_fov}) enables searches for bursts from magnetars like SGR~1935+2154 \citep{Galactic_magnetar}, giant-pulse emitters, rotating radio transients (RRATs; \citealt{RRATs2022}), and other rare phenomena.
...
\begin{figure}[t]
  ...
  \caption[FDM chains and corresponding LO tones]{FDM chains and corresponding LO tones. Also shown is whether the band is upper sideband (USB) or lower sideband (LSB) with respect to the LO frequency.}
   \begin{tabular}{cccc}
    \toprule
    Chain $\Delta\nu_i$ & Frequency range & LO tone & Band \\
    \midrule
    0 & 300--501.6 MHz   & /            & USB \\
...
\begin{figure}[h!]
  ...
  \end{subfigure}
  \caption[CHARTS FDM boards]{CHARTS FDM boards. The differential-input version (a) is a prototype that uses SMA connectors. The RJ45 version (b) incorporates improvements from the differential design and adds RJ45 connectors for an easier integration with the antenna system.}
\end{figure}
...
\begin{figure}
    ...
    \begin{tikzpicture}[font=\tiny, scale=.99, auto]
      ...
      \draw[|<->|] (rtl) -- (rtr) node[midway,above] (rt) {$\text{Realtime processing} \lesssim \qty{10}{\s}$};
      % \draw[|<-] (rtl) -- (rt);
      % \draw[->|] (rt) -- (rtr);
      
    \end{tikzpicture}
    \caption[CHARTS digital signal chain from RFSoC to real-time analysis cluster]{CHARTS digital signal chain from RFSoC to real-time analysis cluster. The system consists of eight RFSoC boards (F-engines) that digitize and channelize the signals from the FDM boards. The data is then distributed to four GPU-based processing nodes (X-engines) that perform beamforming, downsampling, upchannelization, and transient search. A dedicated search node identifies candidates, while an analysis node handles source identification and follow-up actions. Real-time processing latency is kept below 10 seconds to ensure timely detection and response. Figure made by Prof.\ Tomás Cassanelli.}
