...
\begin{table}
      ...
      \begin{tabular}{lll}
        ...
        Bandpass & $\nu=\qtyrange{300}{500}{\mega\hertz}$ \\
        Bandwidth & $\Delta\nu=\qty{201.6}{\mega\hertz}$ \\
        Antenna half-power beamwidth & $\text{HPBW}_\text{a}\approx\qty{\sim100}{\degree}$ \\
        Antenna gain (at zenith) & \num{\sim7} dBi \\
        Polarizations  & $n_\text{p}=\num{1}$\\
        Field-of-view (FoV) & $\Omega_\text{a}\approx\qty{100}{\degree}\times\qty{100}{\degree}$ \\ 
        ...
        Number of beams & $n_\text{b}\approx\num{1000}$ \\
...
\begin{figure}[h!]
  ...
  \caption[Comparison of CHARTS FoV with other radio transient detectors]{Comparison of the field-of-view (FoV) of CHARTS with CHIME/FRB far sidelobe \citep{Lin2024} and BURSTT \citep{Ling_2022}. CHARTS uniquely covers southern latitudes, complementing existing facilities. Its FoV includes the Galactic center, enabling daily monitoring of transient events in the Galaxy and local extragalactic Universe. The gray scale represents the synchrotron background at 300 MHz \citep{BeyondPlanck2023}, with scatter sources indicating magnetars \citep{MagnetarCatalog2014}. Figure made by Prof. Tomás Cassanelli.}
...
Two versions of the antenna have been developed, both differential. The first version uses SMA cables for signal transmission, while the second version employs RJ45 connectors that serve both signal transmission and DC power delivery. All tests presented in this thesis were performed using the SMA version. The RJ45 version, currently under development, will include integrated low-pass filters to improve rejection of out-of-band interference and prevent saturation in subsequent amplification stages.
...
\begin{itemize}
    \item \textbf{Beamforming:} Each node forms $n_b = 1000$ coherent beams using a real-time beamforming kernel. The process includes calibration, frequency upchannelization (from 672 to 16k channels per sub-band, resulting in a frequency resolution of \SI{12.3}{\kilo\hertz}), and accumulation of intensity (Stokes~I) data.
    \item \textbf{Downsampling and data reduction:} The beamformed data are squared, integrated to a time resolution of $t_s \approx \SI{1}{\milli\second}$, and reduced to 4-bit I-Stokes values.
    \item \textbf{Search and detection:} The GPU-based search node performs incoherent dedispersion over a fixed bank of \glspl{dm} up to $\sim$\SI{800}{\parsec\per\centi\meter^3}, followed by a peak-finding kernel that identifies statistically significant transient candidates in beam, time, DM, and SNR space.
\end{itemize}
...
\begin{figure}
    \centering
    \begin{tikzpicture}[font=\footnotesize, scale=.99, auto]
      \coordinate (O) at (0, 0);
      \def\spacing{4.5};
