...
\begin{figure}[h!]
  ...
  \caption[Measured and fitted receiver temperature at Laguna Carén site]{Measured (yellow) and fitted (blue) receiver temperature (here the term receiver includes the amplification box and the 10 dB attenuator) used to characterize the receiver noise profile. The fitted curve smooths out small-scale fluctuations and defines the effective receiver noise profile across the band.}
...
To validate the LO generation module, we performed measurements using an external spectrum analyzer connected directly to the RFSoC DAC output and a high speed oscilloscope. The objective was to assess the spectral purity and frequency accuracy of the digitally synthesized signals. The FFT was computed with 750 points over a 3.28--2457.6~MHz span, yielding a frequency resolution of \SI{3.276}{\mega\hertz}. The DAC output was set to its maximum level for 14-bit samples to maximize the signal-to-noise ratio. Figure \ref{fig:lo_osc_time} shows the time domain data captured of the DAC output through the oscilloscope.

\begin{figure}
	\centering
	\includegraphics{lo_osc_time.pdf}
	\caption[Time domain capture of the LO generation module output]{Time domain capture of the LO generation module output using the RIGOL MSO5104 oscilloscope. The waveform displays the superposition of the four synthesized tones at \SIlist{1064.6;1333.2;1601.8;1863.8}{\mega\hertz}.}
...
\end{equation}
where $X_i(\nu, t_n)$ is the complex FFT output for chain $i$ at frequency channel $\nu$ and time $t_n$, and $N$ is the total number of time samples. The phase was obtained as
\begin{equation}
\phi_{ij}(\nu) = \angle \ S_{ij}(\nu).
\end{equation} 
 Figure~\ref{fig:noise_source_correlation_matrix} shows the resulting correlation matrix. The diagonal elements represent the auto-correlations, while the off-diagonal elements show the cross-correlation phases. 


\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{corr_fdm_load.pdf}
    \caption[Correlation matrix from noise source test]{Correlation matrix obtained from the noise source test. It shows the time averaged cross-correlation phases ($\angle \ S_{ij}$) between all antenna pairs over a 3-minute integration over the full 300-500 MHz bandwidth.}
    \label{fig:noise_source_correlation_matrix}
\end{figure}

Since all chains are fed by the same noise source, the expected phase difference between any pair should be approximately zero (modulo $2\pi$), except for chain~1 which, due to a longer cable, introduces a known delay and exhibits a clear phase offset relative to the others. Small phase shifts are nevertheless observed in the cross-correlations, but they are fairly constant across frequency (minor slopes in chains 0 and 2) attributable to slight differences in cable lengths and phase variations in analog components, especially in the FDM board. Such offsets are common in practical systems and can be corrected through calibration during real observations. The key result is that the system exhibits stable phase coherence across all chains, validating the integrity of the digital processing and confirming that the FDM system maintains the phase stability required for interferometric observations.
...
\begin{figure}[h!]
	...
	\caption[Real, imaginary, and amplitude of cross-correlation for 300.6 MHz during solar transit]{Time series of the real (blue dots) and imaginary (red crosses) parts of the cross-correlation and their amplitude (green dots) for the 300.6 MHz frequency channel.}
...
To validate end-to-end \gls{100gbe} data transmission from the RFSoC to the processing node, we implemented an optical-fiber test using the RFSoC’s QSFP28 interface and a remote server equipped with an Intel E810-CAM2 \gls{100gbe} network interface card. Both devices were configured with matching MAC addresses, IP addresses, and UDP ports.
