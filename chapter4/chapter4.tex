...
These requirements shaped the architecture, and each design decision is discussed in detail in the following sections. 
...
For the streaming implementation, only selected channels from the complete 8192-channel spectrum are transmitted to reduce data rate requirements. Specifically, 672 channels per antenna (corresponding to one complete FDM chain, as described in Table~\ref{tab:fdm_chains}) are extracted from each \SI{3.33}{\micro\second} spectrum frame, while intermediate guard bands and the 0--300~MHz region are discarded. For the 8-antenna CHARTS configuration, this results in a total of 5376 transmitted channels per frame.

The FFT outputs 8 parallel samples per clock cycle, each sample consisting of complex data with 4-bit real and imaginary components after requantization (as described in \S\ref{sec:re-quantization}). The data format per cycle is therefore:
\begin{equation}
\text{Word width} = 8~\text{samples} \times (4 + 4)~\text{bits/sample} = 64~\text{bits}
\end{equation}

At the system clock frequency of \SI{307.2}{\mega\hertz}, this yields a peak input data rate of \SI{19.66}{\giga bit/s}. However, due to the bursty nature of the channelized data---where valid samples arrive in groups of 84 consecutive cycles per chain, followed by idle periods between chains---the sustained average data rate is lower. This average rate can be calculated as:
\begin{equation}
R = \frac{N_{\text{chan/ant}} \times N_{\text{ant}} \times \text{Bits}_{\text{chan}}}{T_{\text{frame}}}
\end{equation}
where $N_{\text{chan/ant}} = 672$ is the number of transmitted channels per antenna, $N_{\text{ant}}$ is the number of antennas, $\text{Bits}_{\text{chan}} = 8$ represents the 4+4 bit complex format, and $T_{\text{frame}} = \SI{3.33}{\micro\second}$ is the spectrum frame period. For the 8-antenna configuration this results in:
\begin{equation}
R_{\text{CHARTS-8}} = \frac{672 \times 8 \times 8~\text{bits}}{\SI{3.33}{\micro\second}} \approx \SI{12.90}{\giga\bit\per\second}
\end{equation}
The architecture is designed to scale to the 32-antenna system, where the data rate increases linearly:
\begin{equation}
R_{\text{CHARTS-32}} = 4 \times R_{\text{CHARTS-8}} \approx \SI{51.61}{\giga\bit\per\second}
\end{equation}

This burst pattern poses specific challenges for the streaming architecture: the system must buffer incoming data during transmission periods and maintain backpressure to prevent overflow while ensuring zero data loss.
...
\begin{enumerate}
    ...
    \item \textbf{Serial-to-Parallel Converter (SIPO)}: Accumulates eight 64-bit words into single 512-bit words matching the 100GbE native interface width. The SIPO employs a time-division multiplexing approach where incoming data is sequentially placed into different segments of a 512-bit parallel register according to a modulo-8 counter. After accumulating all eight segments, the complete word is written to an internal \gls{fifo} block implemented using \gls{bram}.
    ...
    \item \textbf{Packetizer}: Frames the 512-bit words into Ethernet packets by adding protocol metadata (destination IP address and port) and generating end-of-frame (EOF) markers. The packetizer implements a two-state finite state machine (IDLE and READ) that counts transmitted words and asserts EOF after a configurable packet length. For this application, the packet length was set to 84 words to match the number of valid words per FDM chain, resulting in 5376-byte packets. This exceeds the standard Ethernet MTU of 1500 bytes but is supported by jumbo frame configurations in modern 100GbE infrastructure.
    
    The packetizer continuously reads from the SIPO FIFO whenever data is available, incrementing an internal counter with each transfer. When the counter reaches the configured packet length, the module asserts \texttt{tx\_eof} for one cycle to mark the packet boundary and returns to IDLE. The design eliminates forced idle periods between packets in favor of continuous operation with backpressure-based flow control. The packetizer asserts \texttt{sipo\_ready} = 1 whenever in the READ state, allowing data to flow continuously as long as valid data is available. This approach prevents FIFO overflow that would occur with fixed inter-packet delays mismatched to the burst pattern, enabling the system to automatically adapt to varying downstream conditions.
\end{enumerate}
