...
\bottomrule
\end{tabular}
\caption[FPGA resource utilization for the F-engine design]{FPGA resource utilization for the F-engine design, showing the percentage of each major resource consumed on the RFSoC 4x2 device.}
...
The described procedure ensures that the digital re-quantization operates within its optimal dynamic range, minimizing quantization bias and preventing clipping, while maintaining uniform sensitivity across all frequency channels.

\section{Data streaming and packetization}
\label{sec:data_streaming}

Following channelization, the spectral data must be efficiently transmitted to downstream processing systems. This section describes the architecture implemented to convert the narrow parallel output of the FFT into wide packets suitable for \gls{100gbe} transmission, while maintaining zero data loss and adapting to the bursty nature of the spectral output.
...
To ensure contiguous transmission of FDM chains—especially for those that wrap around the FFT output frame boundary, such as chain 3—a dedicated FFT reordering module is implemented in hardware. The FFT core outputs frequency bins in a fixed order: first the positive frequencies (bins 4096-8191, 1228.8–2457.3~MHz), then the negative frequencies (bins 0–4095, 0–1228.5~MHz), over 1024 cycles per frame. However, some FDM chains (e.g., chain 3) span the end and beginning of this frame, resulting in discontinuous data if streamed directly.

To address this, the system employs a double-buffering scheme using two \glspl{bram}. Incoming FFT data is written sequentially into one buffer over a full frame. Once the frame is complete, the buffer switches, and the filled buffer is read out in a reordered fashion: the negative frequencies (second half of the frame) are output first, followed by the positive frequencies (first half). This ``FFT shift'' operation ensures that chains crossing the frame boundary are transmitted as contiguous blocks, simplifying downstream packetization and avoiding gaps in the data stream.

The reordering logic operates as follows: while one buffer is being written with incoming FFT data, the other buffer is read out and streamed. The readout sequence is controlled so that for each frame, the output starts from address 512 (negative frequencies) up to 1023, then continues from address 0 to 511 (positive frequencies). This guarantees that all FDM chains, regardless of their position in the FFT output, are delivered in continuous bursts, with no discontinuities at the frame boundary.

The timing of valid data transmission for each FDM chain within the 1024-cycle FFT output frame is summarized in Table~\ref{tab:streaming_bands}. Each chain's data is transmitted over 84 consecutive cycles, corresponding to its 672 frequency channels, with idle periods in between.
...
This burst-mode transmission pattern introduces specific challenges for the streaming architecture. The system must buffer incoming data during active transmission periods, manage backpressure to prevent buffer overflow, and ensure zero data loss despite the non-uniform arrival of valid data. The subsequent streaming and packetization stages are designed to address these requirements.


\subsection{Streaming architecture}

The streaming system consists of three cascaded modules that perform data width conversion, elastic buffering, and packet formation, as illustrated in Figure~\ref{fig:streaming_architecture}. The dataflow progresses through:

\begin{enumerate}
    \item \textbf{Skid Buffer}: Provides a single-stage pipeline register with backpressure support, decoupling input timing from downstream processing. This module prevents combinational loops in the ready/valid handshake protocol that would otherwise create critical timing paths. When valid input data arrives but the downstream module is stalled, the skid buffer captures the incoming word in an internal register and asserts backpressure by deasserting \texttt{din\_ready}. This design tolerates single-cycle pipeline bubbles while maintaining throughput of one word per cycle during normal operation, adding one clock cycle of latency.
    
    \item \textbf{Serial-to-Parallel Converter (SIPO)}: Accumulates eight 64-bit words into single 512-bit words matching the 100GbE native interface width. The SIPO employs a time-division multiplexing approach where incoming data is sequentially placed into different segments of a 512-bit parallel register according to a modulo-8 counter. After accumulating all eight segments, the complete word is written to an internal \gls{fifo} implemented using \gls{bram}.
    
    The FIFO depth was configured to 512 entries (262~kbit total capacity), providing buffering for four complete spectrum frames. This depth ensures adequate margin to absorb variations in packet transmission rate while preventing overflow during burst periods. The FIFO uses separate read and write pointers with an extra MSB ``wrap-around'' bit to distinguish between full and empty conditions when pointer addresses are equal. An \texttt{almost\_full} threshold provides early backpressure before the FIFO completely fills, preventing deadlock conditions where the accumulator cannot complete a 512-bit word due to insufficient buffer space.
    
    The SIPO propagates backpressure upstream through the \texttt{din\_ready} signal, which is asserted only when the FIFO has space to accept a complete 512-bit word. This ensures zero-loss operation by never accepting data that cannot be buffered. The output operates using a ready/valid protocol where \texttt{dout\_valid} indicates data availability and \texttt{dout\_ready} (from the downstream packetizer) controls FIFO consumption.
    
    \item \textbf{Packetizer}: Frames the 512-bit words into Ethernet packets by adding a header word and generating end-of-frame (EOF) markers. The packetizer implements a three-state finite state machine (IDLE, HEADER, and DATA) that inserts a single header word at the beginning of each packet, followed by the payload data from the SIPO.
    
    The header word currently contains a 64-bit packet sequence counter in the least significant bits, with the remaining 448 bits reserved for future metadata expansion (timestamp, configuration parameters, status flags, etc.). This minimal header enables packet loss detection at the receiver while maintaining low overhead. Each packet consists of 1 header word plus 84 data words (corresponding to one complete FDM chain), resulting in 5440-byte packets that exceed the standard Ethernet MTU but are supported by jumbo frame configurations in modern 100GbE infrastructure.
    
    The state machine operates as follows: after an IDLE period of configurable duration, the packetizer transitions to HEADER state for one cycle to transmit the header word. It then enters DATA state and continuously reads from the SIPO FIFO, incrementing an internal counter with each transfer. When the counter reaches the configured packet length (84 words), the module asserts \texttt{tx\_eof} for one cycle to mark the packet boundary, increments the packet sequence counter, and returns to IDLE. The \texttt{sipo\_ready} signal is asserted only during the DATA state, preventing FIFO consumption during header transmission or idle periods.
\end{enumerate}
...
\begin{tikzpicture}
	...
	\node[shape=rectangle, draw, line width=1pt, minimum width=2.465cm, minimum height=1.965cm] at (12.75, 4.5){} node[anchor=center, align=center, text width=2.077cm, inner sep=6pt] at (12.75, 4.5){\footnotesize Packetizer\\(84 words/pkt)};
...
The packetizer outputs interface directly with CASPER's 100GbE core that handles lower-layer Ethernet framing (preamble, headers, FCS calculation) and physical layer transmission. The configurable IP address and port allow runtime reconfiguration of the data destination without FPGA reprogramming. 
