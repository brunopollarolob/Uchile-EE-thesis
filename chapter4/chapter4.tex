...
\caption[Architecture of the LO generation block]{Architecture of the \gls{lo} generation block. Four tone generators operate in parallel, each producing a digitally synthesized sine wave from lookup tables in block RAM. The phase increment defines the tone frequency, while digital attenuation sets its amplitude. The tones are summed and sent to the DAC tile, which outputs real-valued signals routed to SMA connectors for external analog filtering and distribution to the FDM boards.}
