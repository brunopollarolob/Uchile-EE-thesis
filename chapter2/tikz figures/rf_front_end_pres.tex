\documentclass[beamer,10pt,
    aspectratio=169,
        ...
        %%% antenna
        \node[shape=antenna, xscale=-1, at={(-2,3)}](antenna){};
        ...
        %%% decimator
        \node[draw, thick, shape=rectangle, minimum size=12pt, at={([xshift=50pt]mixer2.center)}, align=center, label={below:Decimador}](decimator){$D \downarrow$};

        %%% digital processing
        \node[draw, thick, shape=rectangle, minimum size=12pt, at={([xshift=50pt]decimator.east)}, align=center](digital-processing){Proc.\\digital};

        %%% Digital/Analog separation line
        \draw[thick, dashed, gray] 
            ($(adc.east)+(90pt,-120pt)$) -- 
            ($(adc.east)+(90pt,+80pt)$)
            node[pos=0.85, above left, gray] {Análogo}
            node[pos=0.85, above right, gray] {Digital};
        
        %% FDM (outside block)
        \node[draw, fit=(rf-bpf) (lo) (if-bpf), thick, rounded corners=5pt, inner ysep=32pt, inner xsep=5pt, yshift=5pt, dashed](fdm-fit){};
        \node[above, inner sep=5pt] at (fdm-fit.north) {``FDM''};
