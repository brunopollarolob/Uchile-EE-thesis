...
\end{equation}
where $\theta$ is the angle between the direction of the radiation and the observer's \gls{los}, $\mathrm{d}\sigma$ is the area element on the telescope surface, $\mathrm{d}\Omega$ is the solid angle element of the source, and $\mathrm{d}\nu$ is the frequency element. It is important to note that the specific intensity is an intrinsic property of the source, while the received power depends on the distance and the effective aperture, observing bandwidth and the integration time.
...
In practice, $T_r$ can be decomposed into the contributions of multiple analog components. For a cascade of $m$ components, each with its own noise figure $F$ and gain $G$ (see Figure \ref{fig:friis}), the effective receiver noise is given by the Friis formula \citep{Friis1944}:  
\begin{equation}
    T_r = T_1 + \frac{T_2}{G_1} + \frac{T_3}{G_1 G_2} + \dots + \frac{T_{m}}{G_1 G_2 \cdots G_{m-1}}.
	\label{eq:friis}
\end{equation}
...
\begin{equation}
    T_\text{sys} = T_r + \underbrace{T_\text{src} + T_\text{cmb} + T_\text{sky} + T_\text{spill}+ \dots}_{T_a} \ ,
\end{equation}
where:
\begin{itemize}
...
\begin{equation}
	...
	\label{eq:radiometer}
\end{equation}
where $t_\text{int}$ is the integration time, $\Delta \nu$ is the bandwidth, and $k_c$ is a proportionality constant that depends on the type of detector. Eq. \ref{eq:radiometer} is fundamental because it shows that sensitivity improves with larger bandwidth and longer integration.  
...
\begin{figure}[t]
	...
	\caption[Heterodyne receiver]{Heterodyne radio telescope receiver. The sky signal is amplified by a \gls{lna}, mixed with a local oscillator to an intermediate frequency, and digitized by an \gls{adc}. A digital down-converter with an \gls{nco} and lowpass filter translates the signal to baseband before decimation and further digital processing. Adapted from \href{https://tikz.net/radiofrequency-frontend/}{TiKz.net}, Rubem Pacelli.}
...
To convert a real, Nyquist-sampled signal $x[n]$ centered at 0 Hz into a quadrature-sampled signal, it can be multiplied by a complex exponential $e^{\mathrm{j}2\pi \nu_0 n}$, resulting in
\begin{equation}
...
\end{tikzpicture}
	\caption[Quadrature sampling block diagram]{Quadrature sampling block diagram, where a real-valued signal $x[n]$ is mixed with a cosine and sine wave at frequency $\nu_0$ to produce the in-phase ($x_I[n]$) and quadrature ($x_Q[n]$) components. The 90° phase shifter ensures the sine wave is in quadrature with the cosine wave. Digital lowpass filters (LPF) remove high-frequency components resulting from the mixing process.}
...
\end{equation}
where $\hat{x}[n]$ is the quantized output, $\text{round}(\cdot)$ denotes rounding to the nearest integer and
$\Delta = 2A / 2^N$ is the step size for an input range $[-A, A]$. This rounding introduces quantization noise, which can be modeled as a uniform random variable in $[-\Delta/2, \Delta/2]$, with average power
\begin{equation}
...
Equivalent behavior is observed in the discrete domain by considering the definition of the DFT for a finite-length signal given in Eq.~\ref{eq:dft}. Here, the summation runs from $n=0$ to $N-1$, indicating that only a finite segment of the signal is analyzed. Mathematically, this finite summation is equivalent to taking the infinite-length \gls{dtft} (Eq. \ref{eq:dtft}) of the product of the signal $x[n]$ and a rectangular (or tophat) window function $\Pi_N[n]$, as shown below:
\begin{align}
    X[k] &=  \text{DTFT} \left( x[n] \cdot \Pi_N[n] \right) \\
    &= \sum_{n=-\infty}^{\infty} x[n] \Pi_N[n] e^{-\mathrm{j} 2\pi n k / N} \\
    &= \sum_{n=0}^{N-1} x[n] e^{-\mathrm{j} 2\pi n k / N} \ , 
\end{align}
...
\begin{equation}
    ...
    \end{cases}.
\end{equation}
In the frequency domain, the rectangular window is Fourier paired with the sinc function (or, more precisely, the Dirichlet kernel in the discrete domain). Consequently, the finite duration of the measurement effectively convolves the ideal spectrum of the signal with this sinc-like function. The undesirable peaks of this function are referred to as sidelobes, which cause spectral leakage.

Windowing functions improve the DFT response by mitigating sidelobe levels at the expense of increasing the main lobe width. This is achieved by multiplying the signal $x[n]$ by a weighting function $w[n]$ before computing the transform:
\begin{equation}
    X_w[k] = \sum_{n=0}^{N-1} w[n] x[n] e^{-\mathrm{j} 2\pi n k / N}.
\end{equation}
According to the convolution theorem for the DFT \citep[see][Chapter~8]{OppenheimSchafer2010}, multiplication in the time domain corresponds to circular convolution in the frequency domain:
\begin{equation}	
    X_w[k] = \frac{1}{N} (W \circledast X)[k],
\end{equation}
where $W[k]$ is the DFT of the window function, and $\circledast$ denotes circular convolution, defined as:
\begin{equation}
    (W \circledast X)[k] = \sum_{m=0}^{N-1} W[m] X[(k - m) \pmod N].
\end{equation}

Among the most widely used windows in digital spectrometry are the Hamming and Hann windows, which offer a good compromise between sidelobe suppression and spectral resolution. Table \ref{tab:window_functions} lists some of the most common window functions along with their mathematical expressions. 
\begin{table}[t]
	\centering
...
\end{tikzpicture}
	\caption[Polyphase filter bank schematic]{Schematic of a PFB with three taps ($P=3$) and an $N$-point FFT. The input signal $x[n]$ is demultiplexed and processed through $N$ parallel sub-filters, where $z^{-1}$ represents a unit delay (one sample period), $h[k]$ denotes the prototype filter coefficients distributed across the polyphase branches, and the mixer symbols represent coefficient multiplication. The outputs of these sub-filters are summed (indicated by the adder nodes) and then transformed by the FFT to produce the final spectral outputs $y[0], y[1], \ldots, y[N-1]$. Adapted from \url{https://casper.berkeley.edu/wiki/The_Polyphase_Filter_Bank_Technique}.}
...
A spectrometer is an instrument designed to measure the spectral composition of signals, such as radio emissions from astronomical sources. It quantifies the \gls{psd}, typically expressed in units of \si{\watt \per \hertz}, which corresponds to the specific flux integrated over the instrument's effective aperture (Eq. \ref{eq:flux_density}). Spectral analysis provides critical insight into the physical properties of radio sources and the characteristics of the intervening medium through which the signals propagate \citep{Price2016spectrometers}.

A correlator is a specialized instrument that extends this concept to interferometric arrays by computing the cross-correlation between signals from different antennas. Further details on correlation are provided in \S\ref{sec:cross_correlation}.
...
\end{equation}
Physically, this function peaks when the lag parameter $\tau$ equals the geometric delay $\tau_g$. The integral can also be expressed as a convolution, which is useful for system analysis:
\begin{align}
...
For an array with $N_a$ antennas, the number of unique baselines is $N_\text{bsl} = N_a(N_a-1)/2$, leading to a computational cost per spectral frame that scales as
\begin{equation}
	\mathcal{O} \left(N_\nu,N_a^2\right),
\end{equation}
...
In addition to correlation, X-engines can perform beamforming by coherently summing antenna voltages with appropriate delays or phase rotations. A straightforward delay-and-sum implementation for $N_b$ beams has a computational complexity of
\begin{equation}
\mathcal{O}\left(N_\nu,N_b,N_a\right),
\end{equation}
which becomes prohibitive for large arrays. However, for regularly sampled arrays, FFT beamforming can be applied to synthesize approximately $N_a$ orthogonal beams with a reduced cost of
\begin{equation}
\mathcal{O}\left(N_\nu,N_a\log_2 N_a\right),
\end{equation}
substantially improving efficiency when $ N_b \sim N_a$ \citep{Masui_2019}.
This approach, however, constrains the array geometry and fixes the array to be stuck in a defined grid concentration but is widely used in large-$N_a$, wide-field interferometers where real-time beamforming is required \citep{Price_2024}.
...
\begin{itemize}
	...
	\item \Glspl{gpu}, originally developed for graphics rendering, have become widely used in high-performance computing. Their highly parallel architecture makes them ideal for large matrix operations and real-time data processing, often serving as backends for cross-correlation and imaging pipelines. In recent years their price has decreased significantly, especially due to the gaming and artificial intelligence industries, making them an attractive option for radio astronomy applications. 
	An example of a pipeline specifically dedicated to \glspl{gpu} is the X-engine of CHIME \citep{recnik2015efficientrealtimedatapipeline, Denman_2015}. 
...
Ethernet has become the standard interconnect technology in modern radio astronomy backends, providing a scalable and cost-effective solution for transporting high data rates between FPGAs, GPUs, and storage systems \citep{Price_2020}. Commercial high-speed variants such as 10GbE (SFP+), 40GbE (QSFP+), and 100GbE (QSFP28; \citealt{IEEE8023bm2015}) are routinely employed, enabling real-time redistribution of digitized signals in FX correlators and spectrometers.  
...
The board also provides multiple high-speed interfaces for data offload and system integration, including a 100GbE QSFP28 port, USB~3.0, and a Mini DisplayPort. External clock and \gls{pps} SMA ports enable precise synchronization and multi-board operation. Power regulation and monitoring are handled by dedicated circuits, ensuring reliable operation with protection against overvoltage, overcurrent, and overheating.
