...
Since their discovery in 1967, pulsars have become fundamental astrophysical laboratories \citep{Lorimer2004}. Their remarkable rotational stability makes them precise cosmic clocks \citep{PulsarTimingArray}, enabling stringent tests of general relativity, the study of dense matter equations of state, and the characterization of the \gls{ism}. Moreover, pulsar timing arrays exploit their long-term stability to search for nanohertz gravitational waves.
...
Another key metric is the spurious-free dynamic range (SFDR), which compares the fundamental tone to the largest spurious component. This is particularly relevant for mitigating intermodulation products caused by strong interferers.  
...
\subsection{X-engine}
The X-stage (or X-engine) of an FX correlator is tasked with computing the cross-correlation products between all pairs of antennas for each frequency channel. After the F-engine has channelized and packetized the data, the X-engine receives these packets, typically over high-speed network links. The X-engine then performs complex multiplications of the frequency-domain data from each antenna pair, accumulating these products over a specified integration time to enhance the signal-to-noise ratio. Also the beamforming operations can be performed in this stage, where signals from multiple antennas are combined to form a coherent beam in a specific direction. The X-engine must handle a significant computational load, especially for large arrays with many antennas, as the number of cross-correlation products scales with the square of the number of antennas. Efficient algorithms and parallel processing techniques are often employed to manage this workload effectively.
...
\caption[RFSoC 4x2 principal components]{Top view of the RFSoC 4x2 FPGA board. SMA connectors provide access to four ADCs and two DACs, while high-speed ports such as QSFP28 enable fast data offload. Extracted from \citet{RealDigital_RFSoC4x2_RefMan_A3}.}
...
The board also provides multiple high-speed interfaces for data offload and system integration, including a 100~Gbps QSFP28 port, Gigabit Ethernet, USB~3.0, and a Mini DisplayPort. External clock and pulse-per-second (PPS) SMA ports enable precise synchronization and multi-board operation. Power regulation and monitoring are handled by dedicated ICs, with protection against overvoltage, overcurrent, and overheating.
