...
\begin{document}
  ...
  El F-engine constituye la primera etapa del backend digital de CHARTS. Implementado sobre la plataforma AMD Xilinx RFSoC 4×2, el sistema digitaliza, canaliza y recuantiza múltiples señales de antena, produciendo flujos espectrales compactos listos para su correlación y la búsqueda de transientes. La arquitectura admite hasta 4 entradas de convertidores analógico-digitales, cada una con 8 señales de antena en un ancho de banda de $\sim$2.5 GHz (para un total de 32 señales digitalizadas), integra osciladores locales digitales para el esquema de multiplexación en frecuencia y transmite los datos procesados mediante enlaces ópticos QSFP28. Como innovación respecto del estado del arte, se incorporan técnicas como FFT compleja para muestreo I/Q y la desmultiplexación de múltiples antenas con un único ADC, dentro de una arquitectura modular, escalable y flexible.

  Pruebas de laboratorio y mediciones en sitio, como la observación del tránsito del sol, validan la integridad de señal, la precisión de la canalización y la eficiencia de transmisión. Se ha validado experimentalmente la transmisión hasta $\sim$12.4 \si{\giga\bit\per\second} con $\sim$5\% de pérdida de paquetes, correspondiente a 8 antenas. El modelo completo de 32 antenas, con una tasa teórica de 51.6 \si{\giga\bit\per\second}, está implementado y listo para ser probado una vez que el software del X-engine y el hardware analógico estén disponibles. Finalmente se discuten los puntos claves que permitirán la escalabilidad del sistema hacia la implementación completa de CHARTS.
  \newpage
