...
\begin{document}
  ...
  El F-engine constituye la primera etapa del backend digital de CHARTS. Implementado sobre la plataforma AMD Xilinx RFSoC 4×2, el sistema digitaliza, canaliza y recuantiza múltiples señales de antena, produciendo flujos espectrales compactos listos para su correlación y la búsqueda de transientes. La arquitectura admite hasta 4 entradas de convertidores analógico-digitales, cada una con 8 señales de antena en un ancho de banda de $\sim\qty{2.5}{\giga\hertz}$ (para un total de \num{32} señales digitalizadas), integra osciladores locales digitales para el esquema de multiplexación en frecuencia y transmite los datos procesados mediante enlaces ópticos QSFP28. Como innovación respecto del estado del arte, se incorporan técnicas como transformada de Fourier (FFT) compleja para muestreo por cuadratura (I/Q) y la desmultiplexación de múltiples antenas con un único convertidor analógico-digital (ADC), dentro de una arquitectura modular, escalable y flexible.

  Pruebas de laboratorio y mediciones en sitio, como la observación del tránsito del sol, validan la integridad de señal, la precisión de la canalización y la eficiencia de transmisión. Se ha validado experimentalmente la transmisión del modelo completo de \num{32} antenas con una tasa de $\sim\qty{52.6}{\giga\bit\per\second}$ y solo $\sim0.01\%$ de pérdida de paquetes. Finalmente se discuten los puntos claves que permitirán la escalabilidad del sistema hacia la implementación completa de CHARTS.
  \newpage
  ...
  The F-engine constitutes the first stage of the CHARTS digital backend. Implemented on the AMD Xilinx RFSoC 4x2 platform, the system digitizes, channelizes, and requantizes multiple antenna signals, producing compact spectral streams ready for correlation and transient search. The architecture supports up to 4 analog-to-digital converter inputs, each with 8 antenna signals over a $\sim\qty{2.5}{\giga\hertz}$ bandwidth (for a total of \num{32} digitized signals), integrates digital local oscillators for the frequency-division multiplexing scheme, and transmits the processed data via optical links QSFP28. As an innovation over the state of the art, techniques such as complex fast Fourier transform (FFT) for quadrature (I/Q) sampling and demultiplexing multiple antennas with a single analog-to-digital converter (ADC) are incorporated within a modular, scalable, and flexible architecture.

  Laboratory tests and on-site measurements, such as the solar transit observation, validate signal integrity, channelization accuracy, and transmission efficiency. Experimental validation has demonstrated transmission up to $\sim\qty{12.9}{\giga\bit\per\second}$ with $\sim$5\% packet loss, corresponding to 8 antennas. The full 32-antenna model, with a theoretical rate of \qty{51.6} {\giga\bit\per\second}, is implemented and ready to be tested once the X-engine software and analog hardware become available. Finally, key points enabling system scalability towards the complete CHARTS implementation are discussed.
