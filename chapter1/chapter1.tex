...
\section{Motivation}
The last decade has established time-domain radio astronomy as one of the most dynamic frontiers of modern astrophysics. By focusing on temporal rather than spatial variability, this field has uncovered a rich variety of energetic phenomena, ranging from pulsars and magnetar bursts to the recently discovered \glspl{frb}. These millisecond-duration flashes are among the brightest events in the radio sky, yet their origins remain one of the outstanding puzzles in high-energy astrophysics \citep{Petroff_2019}.
...
Despite rapid progress, much of the sky, particularly at Southern declinations, remains largely unexplored at the time-domain radio frontier \citep{fender2024fillingradiotransientsgap}. Most transient surveys have been conducted in the Northern Hemisphere, leaving limited coverage of regions such as the \gls{gc} and nearby extragalactic space. Expanding observational capabilities in the south is therefore crucial to complete the global picture of the dynamic radio sky and to uncover new populations of transient sources that may be invisible to current facilities.

The \gls{charts} project was conceived in this context. It aims to deploy a low-frequency interferometric array (300--500~MHz) in the Southern Hemisphere, combining wide sky coverage with sufficient sensitivity to detect, monitor, and help characterize \glspl{frb}, magnetar flares, and other transient radio phenomena. CHARTS also seeks to build the time-domain radio community within South America, where no FRB detections have yet been reported.

At the heart of CHARTS lies a modular signal processing architecture that integrates scalable analog front-ends with a flexible digital backend optimized for transient searches. Within this backend, the F-engine plays a central role: it digitizes and channelizes the signals from each antenna, producing frequency-domain data ready to be correlated in the next stage: the X-engine. The development of an efficient, high-throughput F-engine is therefore a key step toward enabling CHARTS to achieve its scientific objectives.

\section{Objectives}
\label{sec:objectives}
The main objective of this work is to design and implement a spectrometer capable of demultiplexing multiple antenna signals over a \SI{200}{\mega\hertz} bandwidth, using the \glspl{adc} of the Xilinx AMD RFSoC 4x2 platform \citep{RealDigital_RFSoC4x2_RefMan_A3}. This constitutes a fundamental component of the CHARTS digital backend, as it allows a single RFSoC board to digitize and compute the \gls{ft} for up to 32 analog signals from the array of 256 antennas.  
...
\begin{itemize}
    ...
    \item Internally generate the local oscillators required for the FDM mixers, using the \glspl{dac} ports of the RFSoC.
    ...
    \item Synchronize, aggregate, and format the data streams from all 32 antennas, aligning FDM chains, correcting spectral inversions, performing a corner turn, and discarding unused channels.
    \item Integrate the output system with a QSFP28 optical module, enabling data transmission via optical fiber.
\end{itemize}
...
\begin{itemize}
    ...
    \item Chapter 5 presents the testing and validation process for the F-engine, including laboratory experiments and on-sky observations. It details the \gls{rfi} environment measurement campaign, the correlation tests with multiple antennas, and the solar transit observation. The results are analyzed to assess the performance of the F-engine in terms of phase coherence and data integrity.
    \item Chapter 6 concludes the thesis by summarizing the key findings and contributions of this work, and discusses future improvements for scaling the system toward full CHARTS deployment.
\end{itemize}
