...
# Calculate correlation in lag domain for all visibility pairs
correlation_products_lag = {}
for i in range(len(bands)):
    for j in range(len(bands)):
        key = f"corr_{i}{j}"
        reference_visibility = np.mean(correlation_products['corr_12'], axis=1)
        referenced_corr = np.mean(correlation_products[key], axis=1) / reference_visibility
        correlation_products_lag[key] = np.fft.ifft(np.nan_to_num(referenced_corr))

lags = np.fft.fftshift(np.fft.fftfreq(672, d=1/3.33))

# Plot lag-domain correlation products
fig, axes = plt.subplots(8, 8, figsize=(12, 12))
for i in range(8):
    for j in range(8):
        key = f"corr_{i}{j}"
        lag_product = correlation_products_lag[key]
        
        ax = axes[i, j]
        ax.plot(lags, np.fft.fftshift(np.abs(lag_product)), '-.')
        ax.set_ylim(0, np.max(np.abs(lag_product)) * 1.1)

        # Title S_{i,j}
        ax.set_title(r"$R_{{" + f"{i}{j}" + "} }$", fontsize=10)

        if i == 7:
            ax.set_xlabel(r"Lag (\unit{\micro\second})")
        ...
        if j == 0:
            ...
            ax.set_yticks([])
plt.tight_layout()
plt.savefig('../figures/corr_lag_charts8.pdf')
plt.show()